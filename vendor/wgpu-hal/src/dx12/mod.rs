...
impl crate::Surface for Surface {
    ...
    unsafe fn configure(
        ...
        let is_composition_target = !matches!(self.target, SurfaceTarget::WndHandle(_));
        ...
        if !is_composition_target {
            ...
            // We always set ALLOW_TEARING on the HWND swapchain no matter
            // what kind of present mode we want because ResizeBuffers
            // cannot change the swapchain's ALLOW_TEARING flag.
            //
            // Composition swapchains reject this flag combination, so keep
            // the composition path to the DXGI requirements only.
        ...
        let swap_chain = match self.swap_chain.write().take() {
            ...
            None => {
                let swap_effect = match self.target {
                    // Composition swapchains require FLIP_SEQUENTIAL on some Windows setups.
                    SurfaceTarget::Visual(_)
                    | SurfaceTarget::VisualFromWndHandle { .. }
                    | SurfaceTarget::SurfaceHandle(_)
                    | SurfaceTarget::SwapChainPanel(_) => Dxgi::DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL,
                    SurfaceTarget::WndHandle(_) => Dxgi::DXGI_SWAP_EFFECT_FLIP_DISCARD,
                };
        ...
        let waitable = match (
            ...
            device.options.latency_waitable_object.clone(),
        ) {
            (true, _) => None,
            (false, wgt::Dx12UseFrameLatencyWaitableObject::Wait)
            | (false, wgt::Dx12UseFrameLatencyWaitableObject::DontWait) => {
                Some(unsafe { swap_chain.GetFrameLatencyWaitableObject() })
            }
            (false, wgt::Dx12UseFrameLatencyWaitableObject::None) => None,
        };
